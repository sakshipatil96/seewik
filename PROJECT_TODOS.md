...
- [ ] Remove **Use my location for discovery** from the Initiative creation form. Creating an Initiative must use the organiser-confirmed meeting-point map/pin, not assume the organiser’s current device location is the activity location.
...
- [ ] Do not leave **Publish activity** silently disabled when coordinates are missing. Show the exact unmet requirement beside the action, preserve completed form fields, and provide the structured map/pin fallback when browser geolocation is unavailable or denied.
...
- [ ] On a filed report’s citizen-facing **Report details** screen, hide the internal **Route** identifier (for example `NMC-PW-POTHOLE-v0.2`) and **Pack** version. Preserve both values in the immutable record and administrative/debug evidence; they do not need to be shown to the citizen.
